...
// Series detail page with all information and recommendations
'use client';
...
export default function SeriesDetailPage({
  ...
  const fetchRecommendations = async () => {
    try {
      const response = await fetch(
        `/api/recommendations?type=similar&seriesId=${params.id}`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const checkIfSaved = async () => {
    try {
      const response = await fetch('/api/user?type=saved');
      const data = await response.json();
      if (data.success) {
        setIsSaved(data.data.includes(parseInt(params.id)));
      }
    } catch (error) {
        ...
        <div className="relative h-full flex items-end pb-12 px-4">
          <div className="max-w-7xl mx-auto w-full">
            <motion.div
              ...
              className="space-y-4"
            >
              <h1 className="text-4xl md:text-6xl font-display font-black text-glow-red">
                {series.name}
              </h1>
              
              {series.tagline && (
                <p className="text-xl md:text-2xl text-neon-blue font-body italic">
                  "{series.tagline}"
                </p>
              )}
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm md:text-base">
                ...
                {series.first_air_date && (
                  <span className="glass-card px-3 py-1 font-body">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 space-y-12">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <motion.div
            ...
            className="relative aspect-[2/3] glass-card overflow-hidden"
          >
            {series.poster_path && (
              <Image
          ...
          >
            <div>
              <h2 className="text-2xl font-display font-bold text-neon-red mb-4">
                Overview
              </h2>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {series.genres?.map((genre) => (
                  <span
                    key={genre.id}
                    className="glass-card px-3 py-1 text-sm font-body"
                  >
                    {genre.name}
                  </span>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <h4 className="text-sm font-display font-bold text-gray-400 mb-1">
                  Status
                </h4>
        ...
        {series.credits && series.credits.cast.length > 0 && (
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-2xl font-display font-bold text-neon-red mb-6">
              Cast
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {series.credits.cast.slice(0, 12).map((actor) => (
                <div key={actor.id} className="glass-card overflow-hidden">
                  ...
                  <div className="p-2">
                    <p className="font-display font-bold text-sm text-white line-clamp-1">
                      {actor.name}
                    </p>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/explore">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="text-neon-blue font-display font-semibold hover:text-glow-blue"
                >
                  View More →
                </motion.button>
              </Link>
